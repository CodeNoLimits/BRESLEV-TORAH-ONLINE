...
export class BreslovDiagnostic {
      ...
      try {
        ...
        if (text) {
          ...
          if (text.text && Array.isArray(text.text)) {
            console.log(
              `[BreslovDiagnostic] Analyzing ${text.text.length} text segments:`,
            );
            text.text.forEach((segment: any, i: number) => {
              if (i < 5) {
                // Only log first 5 for brevity
                console.log(
                  `  Segment ${i}: type=${typeof segment}, length=${segment ? segment.length : "null"}, content="${segment ? segment.substring(0, 50) : "null"}..."`,
                );
              }
            });
          }
          ...
          if (text.he && Array.isArray(text.he)) {
            console.log(
              `[BreslovDiagnostic] Analyzing ${text.he.length} Hebrew segments:`,
            );
            text.he.forEach((segment: any, i: number) => {
              if (i < 5) {
                // Only log first 5 for brevity
                console.log(
                  `  Hebrew ${i}: type=${typeof segment}, length=${segment ? segment.length : "null"}, content="${segment ? segment.substring(0, 50) : "null"}..."`,
                );
              }
            });
          }
          ...
          if (text.text && Array.isArray(text.text)) {
            ...
            console.log(
              `[BreslovDiagnostic] Found ${englishCount} English segments in text.text array`,
            );
          }

          if (text.he && Array.isArray(text.he)) {
            hebrewCount = text.he.filter(
              (segment: any) =>
                segment &&
                typeof segment === "string" &&
                segment.trim().length > 0,
            ...
            console.log(
              `[BreslovDiagnostic] Found ${hebrewCount} Hebrew segments in text.he array`,
            );
          }
        ...
        console.log(
          `[BreslovDiagnostic] ${ref}: ${diagnostic.status} (EN: ${diagnostic.englishSegments}, HE: ${diagnostic.hebrewSegments})`,
        );
      } catch (error) {
        results.push({
          ref,
  ...
  private static printSummary(results: BookDiagnostic[]): void {
    ...
    successful.forEach((book) => {
      console.log(
        `   - ${book.displayName}: ${book.englishSegments} EN, ${book.hebrewSegments} HE`,
      );
    });
