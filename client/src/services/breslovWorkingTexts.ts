...
export class WorkingBreslovService {
  ...
  async getWorkingText(ref: string): Promise<any> {
    ...
    try {
      ...
      console.log(`[WorkingBreslovService] DEBUG: Structure for ${ref}:`, {
        ...
        firstVersionKeys: data.versions?.[0]
      ...
      if (
        ...
        for (const version of data.versions) {
          if (
            ...
            const hasContent = version.text.some(
              (segment: any) =>
                segment &&
            ...
            if (hasContent) {
              ...
              console.log(
                `[WorkingBreslovService] ✅ Successfully loaded ${ref} (${version.language})`,
              );
              return data;
            }
          }
      ...
      if (data.text && Array.isArray(data.text) && data.text.length > 0) {
        const hasContent = data.text.some(
          (segment: any) =>
        ...
        if (hasContent) {
          this.cache.set(ref, data);
          console.log(
            `[WorkingBreslovService] ✅ Successfully loaded ${ref} (legacy format)`,
          );
          return data;
        }
      }
      ...
      console.warn(
        `[WorkingBreslovService] ❌ No substantial content in ${ref}`,
      );
      return null;
    } catch (error) {
  ...
  async getAllWorkingTexts(): Promise<WorkingBreslovText[]> {
    ...
    console.log(
      `[WorkingBreslovService] Found ${workingTexts.length} working texts out of ${VERIFIED_BRESLOV_TEXTS.length}`,
    );
    return workingTexts;
  }
