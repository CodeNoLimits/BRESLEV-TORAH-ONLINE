...
const extractAllTextRefs = (nodes: any[]): SefariaIndexNode[] => {
  ...
  for (const node of nodes) {
    ...
    console.log("[SefariaProxy] Processing node:", {
      title: node.title,
      ref: node.ref,
      hasContents: !!node.contents,
    });
...
const getFullBreslovLibrary = (fullIndex: any[]): SefariaIndexNode[] => {
  try {
    ...
    const chasidutCategory = fullIndex.find(
      (cat) => cat.category === "Chasidut",
    );
    if (!chasidutCategory) {
    ...
    const breslovCategory = chasidutCategory.contents?.find(
      (subCat: any) => subCat.category === "Breslov",
    );
    if (!breslovCategory) {
      console.log("[SefariaProxy] Breslov category not found in Chasidut");
      console.log(
        "[SefariaProxy] Available categories:",
        chasidutCategory.contents?.map((c: any) => c.category),
      );
      return [];
    }
    ...
    console.log(
      `[SefariaProxy] Extracted ${breslovBooks.length} Breslov books`,
    );
...
export const getBreslovIndex = async (): Promise<SefariaIndexNode[]> => {
  ...
  console.log(
    `[SefariaProxy] Complete Breslov library: ${breslovBooks.length} authentic books loaded`,
  );
  return breslovBooks;
};
...
export const getTextContent = async (ref: string): Promise<SefariaText> => {
  console.log(
    `[SefariaProxy] Using new complete text extraction system for: ${ref}`,
  );
  ...
  if (bookTitle) {
    try {
      ...
      console.log(
        `[SefariaProxy] Fetching complete Breslov text from: ${endpoint}`,
      );
      ...
      if (response.ok) {
        ...
        console.log(
          `[SefariaProxy] COMPLETE Breslov text retrieved - EN: ${completeText.text.length} segments, HE: ${completeText.he.length} segments`,
        );
        return completeText;
      }
    } catch (error) {
      console.log(
        `[SefariaProxy] Complete text extractor failed, falling back to standard proxy`,
      );
    }
  }
  ...
  const response = await fetch(
    `${BASE_URL}/texts/${encodeURIComponent(ref)}?lang=en&context=0&commentary=0&alts=1`,
  );
  if (!response.ok) {
  ...
  console.log(
    `[SefariaProxy] Standard text retrieved - EN: ${result.text.length} segments, HE: ${result.he.length} segments`,
  );
  return result;
};

// Cache management
let breslovIndexCache: { data: SefariaIndexNode[]; timestamp: number } | null =
  null;
...
export const getCachedBreslovIndex = async (): Promise<SefariaIndexNode[]> => {
  if (
    breslovIndexCache &&
    Date.now() - breslovIndexCache.timestamp < CACHE_DURATION
  ) {
