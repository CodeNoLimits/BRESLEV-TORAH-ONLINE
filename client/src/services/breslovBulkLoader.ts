...
class BreslovBulkLoader {
  ...
  async loadCompleteLibrary(
    onProgress?: (progress: BulkLoadProgress) => void,
  ): Promise<CompleteBreslovLibrary> {
    console.log("[BreslovBulkLoader] Starting complete library load...");

    // Calculate totals
    this.progress.total = this.COMPLETE_BRESLOV_BOOKS.reduce(
      (sum, book) => sum + book.segments,
      0,
    );
    ...
    for (const bookConfig of this.COMPLETE_BRESLOV_BOOKS) {
      ...
      console.log(
        `[BreslovBulkLoader] Loading ${bookConfig.title} (${bookConfig.segments} texts)...`,
      );
      ...
      try {
        ...
        console.error(
          `[BreslovBulkLoader] Error loading ${bookConfig.title}:`,
          error,
        );
        this.progress.errors.push(
          `Failed to load ${bookConfig.title}: ${error}`,
        );
      }
    }

    console.log(
      `[BreslovBulkLoader] Complete library loaded: ${library.metadata.totalSegments} total segments`,
    );
    return library;
  }
  ...
  private extractAllSegments(data: any): string[] {
      ...
      for (const version of data.versions) {
        ...
        if (version.he) {
          if (Array.isArray(version.he)) {
            segments.push(
              ...version.he.filter((seg: any) => seg && seg.trim().length > 0),
            );
    ...
    if (data.text) {
      if (Array.isArray(data.text)) {
        segments.push(
          ...data.text.filter((seg: any) => seg && seg.trim().length > 0),
        );
    ...
    if (data.he) {
      if (Array.isArray(data.he)) {
        segments.push(
          ...data.he.filter((seg: any) => seg && seg.trim().length > 0),
        );
