...
export const testBookAvailability = async (
  book: SefariaBook,
): Promise<boolean> => {
  try {
    ...
    if (book.firstSection) {
      const response = await fetch(
        `/api/sefaria/v3/texts/${book.firstSection}?context=0&commentary=0&pad=0&wrapLinks=false`,
      );
      if (response.ok) {
    ...
    const response = await fetch(
      `/api/sefaria/v3/texts/${book.key}?context=0&commentary=0&pad=0&wrapLinks=false`,
    );
    if (response.ok) {
      const data = await response.json();
      return data.text && Array.isArray(data.text) && data.text.length > 0;
...
export const getSection = async (ref: string): Promise<any> => {
  try {
    const response = await fetch(
      `/api/sefaria/v3/texts/${ref}?context=0&commentary=0&pad=0&wrapLinks=false`,
    );
    if (response.ok) {
      const data = await response.json();
      return data;
