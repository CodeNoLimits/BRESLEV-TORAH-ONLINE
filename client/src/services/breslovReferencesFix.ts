...
export const getBreslovReference = (title: string): BreslovReference | null => {
  return BRESLOV_REFERENCES_FIXED.find(ref => 
    ref.title === title || 
...
export const verifyBreslovBook = async (ref: BreslovReference): Promise<boolean> => {
  try {
    // Test avec la clé principale
    const response = await fetch(`/api/sefaria/v3/texts/${ref.sefariaKey}?context=0&commentary=0&pad=0&wrapLinks=false`);
    if (response.ok) {
    ...
    for (const altKey of ref.alternativeKeys) {
      const altResponse = await fetch(`/api/sefaria/v3/texts/${altKey}?context=0&commentary=0&pad=0&wrapLinks=false`);
      if (altResponse.ok) {
...
export const getAvailableBreslovBooks = async (): Promise<BreslovReference[]> => {
