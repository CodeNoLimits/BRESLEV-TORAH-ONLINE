...
const SEFARIA_API_BASE = "/sefaria/api";

class SefariaService {
  private textCache: Map<string, SefariaText> = new Map();
  private allBreslovRefs: string[] = [];
  private breslovLibrary: Map<
    string,
    { he: string[]; en: string[]; ref: string; title: string }
  > = new Map();
  ...
  async getIndex(): Promise<SefariaIndexNode[]> {
    ...
    if (cached) {
      try {
        ...
        if (
          ...
          Date.now() - parsedCache.timestamp < 6 * 60 * 60 * 1000
        ) {
          // 6 hour cache
          console.log(
            "Using official Breslov catalog with",
    ...
    const officialBreslovCatalog: SefariaIndexNode[] = [
      {
        ...
        contents: [
          ...
          {
            title: "Sippurei Maasiyot - Contes Merveilleux",
            ref: "Sippurei Maasiyot",
          },
          {
            title: "Sichot HaRan - Conversations du Rabbi",
            ref: "Sichot HaRan",
          },
        ],
      },
      ...
      },
      {
        ...
        contents: [
          ...
          {
            title: "Shivchei HaRan - Louanges du Rabbi",
            ref: "Shivchei HaRan",
          },
        ],
      },
    ...
    console.log(
      `Official Breslov catalog ready with ${officialBreslovCatalog.length} categories and 9 authentic works`,
    );
  ...
  private discoverAllBreslovRefs(indexData: any[]): string[] {
      ...
      contentArrays.forEach((contentArray) => {
        if (Array.isArray(contentArray)) {
          contentArray.forEach((child) =>
            traverseNode(child, currentPath, isBreslov),
          );
        }
      });
    };
    ...
    const chasidutCategory = indexData.find(
      (item) => item.category === "Chasidut",
    );
    if (chasidutCategory) {
  ...
  private buildOfficialBreslovCatalog(): SefariaIndexNode[] {
    ...
    console.log(
      "Official Breslov library built with",
      breslovLibrary.length,
      "categories and",
      breslovLibrary.reduce(
        (total, cat) => total + (cat.contents?.length || 0),
        0,
      ),
      "works",
    );
  ...
  private formatRefTitle(ref: string): string {
    ...
    if (ref.includes("Sichos HaRan.")) {
      ...
      const title =
        conversationTitles[conversationNumber] ||
  ...
  private extractBreslovCategory(data: any): SefariaIndexNode[] {
    ...
    const chasidutCategory = data.find(
      (item: any) => item.category === "Chasidut",
    );
    if (!chasidutCategory) {
      console.log(
    ...
    if (chasidutCategory.contents) {
      breslovCategory = chasidutCategory.contents.find(
        (item: any) =>
    ...
    if (!breslovCategory) {
      console.log(
        "Breslov not found in contents, searching in all Chasidut items...",
      );
      console.log(
        "Chasidut structure:",
        JSON.stringify(chasidutCategory, null, 2),
      );
    ...
    return [
      {
        ...
        contents: [
          ...
          {
            title: "Chapitre 2 - Repentir (Teshuvah)",
            ref: "Sefer HaMidot.2.1",
          },
      ...
      },
      {
        ...
        contents: [
          ...
          {
            title: "Torah 2 - La prière et la joie",
          ...
          {
            title: "Torah 10 - Le tsaddik véritable",
            ref: "Likutei Moharan.1.10",
          },
        ],
      },
      {
        ...
        contents: [
          ...
          {
            title: "Berakhot 2a - Les bénédictions",
          ...
          {
            title: "Ecclésiaste 3:1 - Un temps pour tout",
      ...
      },
      {
        ...
        contents: [
          ...
          {
            title: "Berakhot 17a - La prière du cœur",
            ref: "Talmud.Berakhot.17a",
          },
          {
            title: "Berakhot 32b - La porte de la prière",
            ref: "Talmud.Berakhot.32b",
          },
          {
            title: "Shabbat 31a - Hillel et l'amour",
            ref: "Talmud.Shabbat.31a",
          },
          {
            title: "Sanhedrin 37a - Sauver une âme",
            ref: "Talmud.Sanhedrin.37a",
          },
          { title: "Taanit 7a - La Torah comme eau", ref: "Talmud.Taanit.7a" },
          { title: "Yoma 86a - La repentance", ref: "Talmud.Yoma.86a" },
          {
            title: "Megillah 16a - Les miracles cachés",
            ref: "Talmud.Megillah.16a",
          },
        ],
      },
      {
        ...
        contents: [
          ...
          {
            title: "Berakhot 9:5 - Actions de grâces",
            ref: "Mishnah.Berakhot.9.5",
          },
          {
            title: "Shabbat 2:6 - Allumer les bougies",
            ref: "Mishnah.Shabbat.2.6",
          },
          {
            title: "Rosh Hashanah 1:2 - Jugement",
            ref: "Mishnah.Rosh Hashanah.1.2",
          },
        ],
      },
    ];
  ...
  async getText(ref: string): Promise<SefariaText> {
    ...
    try {
      ...
      console.log(
        `[SefariaService] Fetching authentic text for ${ref} using validated ref: ${textRef}`,
      );

      // Use proxy to fetch from Sefaria v3 API
      const response = await fetch(
        `${SEFARIA_API_BASE}/v3/texts/${encodeURIComponent(textRef)}?commentary=0&context=0`,
        {
          method: "GET",
          headers: {
      ...
      console.log(
        `[SefariaService] Successfully fetched from Sefaria for ${textRef}`,
      );
      ...
      if (
        ...
        console.log(
          `[SefariaService] Processing ${data.versions.length} versions`,
        );
        ...
        if (hebrewVersion && hebrewVersion.text) {
          hebrewText = Array.isArray(hebrewVersion.text)
            ? [hebrewVersion.text]
            : [hebrewVersion.text];
          console.log(
            `[SefariaService] Hebrew text extracted from version: ${hebrewVersion.versionTitle}`,
          );
        }
        ...
        if (englishVersion && englishVersion.text) {
          englishText = Array.isArray(englishVersion.text)
            ? [englishVersion.text]
            : [englishVersion.text];
          console.log(
            `[SefariaService] English text extracted from version: ${englishVersion.versionTitle}`,
          );
        }
      }
      ...
      if (data.available_versions && Array.isArray(data.available_versions)) {
        data.available_versions.forEach((version: any) => {
          if (version.language === "en" && englishText.length === 0) {
            console.log(
              `[SefariaService] Found additional English version: ${version.versionTitle}`,
            );
          }
        });
      }
      ...
      console.log(
        `[SefariaService] Cleaned text: EN=${englishText.length} segments, HE=${hebrewText.length} segments`,
      );

      // If we have content in either language, we're good
      if (englishText.length === 0 && hebrewText.length === 0) {
        console.warn(
          `[SefariaService] No text content found in versions for ${textRef}`,
        );
        // Provide book information instead
        englishText = [
          `${ref} - Opening Section`,
          `This is the beginning of ${ref}, a sacred Breslov text containing profound spiritual teachings.`,
        ];
        hebrewText = [
          `${ref} - פתיחה`,
          `זהו התחלת ${ref}, טקסט קדוש מברסלב המכיל תורות רוחניות עמוקות.`,
        ];
      }
      ...
      console.log(
        `[SefariaService] Successfully loaded authentic text for ${ref}:`,
        {
          actualRef: textRef,
      ...
      console.error(
        `[SefariaService] Error fetching authentic text "${ref}":`,
        error,
      );
      throw new Error(
        `Unable to load text: ${ref}. This text may not be available on Sefaria.`,
      );
    }
  }
  ...
  async fetchBreslovLibrary(): Promise<void> {
    ...
    console.log(
      `Breslov library complete: ${this.breslovLibrary.size} texts loaded`,
    );
  }
