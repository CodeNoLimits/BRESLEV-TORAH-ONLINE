...
class BreslovCompleteService {
  ...
  async getAuthenticText(ref: string): Promise<any> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`[BreslovComplete] Failed to fetch ${ref}: HTTP ${response.status}`);
        return null;
      }

      const data = await response.json();
      
      // Vérifier le contenu authentique dans la structure Sefaria v3
        ...
        for (const version of data.versions) {
          if (version.text && Array.isArray(version.text) && version.text.length > 0) {
            ...
            if (hasContent) {
              ...
              console.log(`[BreslovComplete] ✅ Loaded: ${ref} (${version.language})`);
              return data;
            }
          }
        }
      ...
      console.warn(`[BreslovComplete] No content found for ${ref}`);
      return null;
    } catch (error) {
