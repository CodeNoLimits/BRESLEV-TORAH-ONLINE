...
class SefariaService {
  ...
  async getText(ref: string): Promise<SefariaText> {
    ...
    console.log(
      `[SefariaService] Fetching COMPLETE authentic text via proxy for ${ref}`,
    );
    ...
    if (
      ...
      throw new Error(
        `CRITICAL: Incomplete authentic text received for ${ref}. Refusing to display partial content.`,
      );
    }
    ...
    console.log(
      `[SefariaService] Cached COMPLETE authentic text for ${ref}: EN(${text.text.length}) HE(${text.he.length})`,
    );
