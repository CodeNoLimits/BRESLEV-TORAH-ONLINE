    ...
    sections: [
      {
        ...
        title: "Story 1: The Lost Princess",
        number: "1",
      },
      {
        ref: "Sippurei Maasiyot 2",
        title: "Story 2: The King and the Emperor",
        number: "2",
      },
      {
        ref: "Sippurei Maasiyot 3",
        title: "Story 3: The Cripple",
        number: "3",
      },
      {
        ref: "Sippurei Maasiyot 4",
        title: "Story 4: The Bull and the Ram",
        number: "4",
      },
      {
        ref: "Sippurei Maasiyot 5",
        title: "Story 5: The Prince of Gems",
        number: "5",
      },
    ],
  },
...
export async function extractCompleteBook(
  bookTitle: string,
  sectionNumber: string | null = null,
): Promise<BreslovTextResult> {
  console.log(
    `[FullTextExtractor] Extracting complete content for ${bookTitle}${sectionNumber ? ` section ${sectionNumber}` : ""}`,
  );
  ...
  try {
    ...
    if (sectionNumber) {
      ...
      console.log(
        `[FullTextExtractor] Fetching complete section: ${sectionRef}`,
      );
      ...
      for (const approach of approaches) {
        try {
          const response = await fetch(
            `https://www.sefaria.org/api/texts/${encodeURIComponent(approach)}?lang=both&context=1&commentary=0&multiple=1`,
          );
          if (response.ok) {
            ...
            const extractText = (textData: any): string[] => {
              ...
              if (typeof textData === "string")
                return [textData.trim()].filter((t) => t);
            ...
            if (english.length > completeEnglishText.length) {
              ...
              console.log(
                `[FullTextExtractor] Better content found with approach: ${approach} (${english.length} segments)`,
              );
            }
          ...
          console.log(
            `[FullTextExtractor] Approach ${approach} failed:`,
            error.message,
          );
        }
      }
      ...
      console.log(
        `[FullTextExtractor] Extracting book overview for ${bookTitle}`,
      );

      for (
        let section = 1;
        section <= Math.min(5, book.totalSections || 5);
        section++
      ) {
        try {
          const sectionRef = `${book.baseRef} ${section}`;
          const response = await fetch(
            `https://www.sefaria.org/api/texts/${encodeURIComponent(sectionRef)}?lang=both&context=1`,
          );
          ...
          console.log(
            `[FullTextExtractor] Failed to fetch section ${section}:`,
            error.message,
          );
        }
      }
    ...
    console.log(
      `[FullTextExtractor] COMPLETE extraction result - EN: ${finalEnglish.length} segments, HE: ${finalHebrew.length} segments`,
    );

    if (finalEnglish.length === 0) {
      throw new Error(
        `No English text extracted for ${bookTitle}${sectionNumber ? ` section ${sectionNumber}` : ""}`,
      );
    }
    ...
    return {
      ...
      text: finalEnglish,
      he:
        finalHebrew.length > 0
          ? finalHebrew
          : [
              `Hebrew text for ${bookTitle}${sectionNumber ? ` section ${sectionNumber}` : ""}`,
            ],
