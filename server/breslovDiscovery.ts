  ...
  for (const book of BRESLOV_BOOKS) {
    try {
      ...
      const indexResponse = await fetch(
        `https://www.sefaria.org/api/index/${book.ref}`,
      );
      if (indexResponse.ok) {
        const indexData = await indexResponse.json();
        console.log(
          `[BreslovDiscovery] Found index for ${book.title}:`,
          indexData.title,
        );
        ...
        for (const testRef of testRefs) {
          try {
            const textResponse = await fetch(
              `https://www.sefaria.org/api/v3/texts/${encodeURIComponent(testRef)}?context=0&commentary=0`,
            );
            if (textResponse.ok) {
              ...
              if (textData.versions && textData.versions.length > 0) {
                validRef = testRef;
                console.log(
                  `[BreslovDiscovery] Valid reference found for ${book.title}: ${validRef}`,
                );
                break;
              }
            }
      ...
      console.log(
        `[BreslovDiscovery] Could not verify ${book.title}: ${error}`,
      );
